...
<article class="flex flex-col gap-8">
	...
	{#if post.coverImage && post.coverImage.trim() !== ''}
		<section class="flex w-full justify-center lg:px-4">
			<div class="max-h-115 w-full max-w-312 overflow-hidden bg-gray-200 lg:rounded-xl">
				<img src={post.coverImage} alt={post.title} class="h-full w-full object-cover object-top" />
			</div>
		</section>
	{/if}
	...
	<div class="flex w-full flex-col items-center gap-8 px-4 pt-16 pb-10">
		<div class="flex w-full max-w-312 flex-col gap-2 md:px-6">
			<div
				class="prose prose-lg prose-headings:text-gray-900 prose-p:text-gray-700 prose-li:text-gray-700 prose-strong:text-gray-900 prose-code:text-gray-900 prose-code:bg-gray-100 prose-code:px-1 prose-code:rounded prose-pre:bg-gray-900 prose-pre:text-gray-100 prose-a:text-blue-600 prose-a:no-underline hover:prose-a:underline max-w-none"
			>
				<content></content>
			</div>
		</div>
