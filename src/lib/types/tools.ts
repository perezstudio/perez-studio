...
export interface Tool {
  ...
  icon?: string;
  logo?: string;
  category: 'frontend' | 'backend' | 'database' | 'design' | 'devops' | 'mobile' | 'testing' | 'other';
  website?: string;
...
// Helper functions
export function getProjectsByTool(projects: PortfolioPostWithTools[], toolSlug: string): PortfolioPostWithTools[] {
  return projects.filter(project => 
    project.tools.some(tool => tool.slug === toolSlug)
  );
}
...
export function getRelatedTools(projects: PortfolioPostWithTools[], currentToolSlug: string): Tool[] {
